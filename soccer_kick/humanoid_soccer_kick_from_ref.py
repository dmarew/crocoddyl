...
from soccer import SoccerProblem
import numpy as np
...
def playback_trajectory(data, display, playback_speed=1.0, repeat=False):
    ...
    while True:
        ...
        try:
            ...
            break
def generate_kick_trajectory(config_file, prev_result=None):
    with open(config_file) as file:
        config = yaml.load(file, Loader=yaml.FullLoader)
        robot_info = config["robot_info"]
        traj_params = config["traj_params"]
        debug_info = config["debug_info"]
        jumping_params = config["jumping_params"]
        targetLocation = np.array(config["kick_params"]["targetLocation"])
        kickKnots = config["kick_params"]["kickKnots"]
        recoveryKnots = config["kick_params"]["recoveryKnots"]
    ...
    display.robot.viewer.gui.applyConfiguration('world/point', targetLocation.tolist() + [0., 0., 0., 1.])  # xyz+quaternion
    ...
    problem = gait.createKickProblem(x0, targetLocation, timeStep, kickKnots, recoveryKnots)
    # Problem definition
    ...
    if prev_result is not None:
        xs = prev_result["xs"] + xs

        for key, value in ps.items():
            prev_result["ps"][key] +=  value
        ps = prev_result["ps"]
        dts = prev_result["dts"] + dts
        fs = prev_result["fs"] + fs
        us = prev_result["us"] + us
    result = {}
    result['xs'] = xs
    result['us'] = us
    result['ts'] = ts
    ...
    ref_trajectory_file = os.path.join(soccer_res_dir, debug_info['experiment_name'] + '_kick.pkl')
    ...
    return xs[-1]
def generate_jump_step_trajectory(config_file):
    with open(config_file) as file:
        config = yaml.load(file, Loader=yaml.FullLoader)
        robot_info = config["robot_info"]
        traj_params = config["traj_params"]
        ...
        jump_height = jumping_params["height"]
        jump_step = np.array(jumping_params["step"])
    ...
    # landing_problem = gait.createTakeOffProblem(x0, jump_height, jump_step, timeStep, groundKnots, flyingKnots, final=True)

    # xsjump = solver.xs
    # usjump = solver.us

    # solver = crocoddyl.SolverBoxFDDP(landing_problem)
    ...
    # models = solver.problem.runningModels.tolist() + [solver.problem.terminalModel]
    # us = usjump + [
    #     np.zeros(0) if isinstance(m, crocoddyl.ActionModelImpulseFwdDynamics) else m.differential.quasiStatic(
    #         d.differential, rmodel.defaultState) for m, d in list(zip(models, solver.datas()))[len(usjump):-1]
    # ]
    # solver.setCandidate(xs=xsjump + [rmodel.defaultState] * (len(solver.models) - len(xsjump)), us=us)
    ...
    if not os.path.exists(soccer_res_dir):
        os.makedirs(soccer_res_dir)
    ref_trajectory_file = os.path.join(soccer_res_dir, debug_info['experiment_name'] + '_jump.pkl')
    # check if user wants to save the reference trajectory
    prompt = f"Save the trajectory to {ref_trajectory_file}? [y/n]"
    ans = input(prompt)
    if ans == 'y':
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Soccer demo')
    parser.add_argument('--playback', action='store_true', help='playback the saved trajectory')
    parser.add_argument('--playback-speed', type=float, default=1.0, help='playback speed')
    parser.add_argument('--config-file', type=str, default='soccer_config.yaml', help='path to the config file')
    parser.add_argument('--repeat', action='store_true', help='repeat the playback')
    parser.add_argument('--kick-phase', type=str, default='jump', help='kick phase')
    ...
    if args.playback:
        try:
            with open(args.config_file) as file:
                ...
                debug_info = config['debug_info']
                soccer_res_dir = os.path.join(os.path.dirname(__file__), debug_info['save_dir_path'])
                ref_trajectory_file = os.path.join(soccer_res_dir, debug_info['experiment_name'] + f'_{args.kick_phase}.pkl')
        ...
        try:
            ...
            playback_trajectory(traj_data, display, playback_speed=1.0/args.playback_speed, repeat=args.repeat)
        except:
            print("Exiting playback")
    else:
        print("Generating new reference trajectory")
        # generate_scoccer_kick_trajectory(config_file=args.config_file)
        print("config file is ", args.config_file)
        result = generate_jump_step_trajectory(config_file=args.config_file)
        generate_kick_trajectory(config_file=args.config_file, prev_result=result)
